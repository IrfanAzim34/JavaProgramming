...
public class MapPractice {
    ...
    public static void main(String[] args) {
        ...
        List<Map<String,Double>> listOfUnit = new ArrayList<>();
        listOfUnit.addAll(Arrays.asList(unit1,unit2,unit3,unit4,unit5,unit6));

        for (Map<String, Double> eachUnit : listOfUnit) {
        ...
        String maxHoursTopic = "";
        String minHoursTopic = "";

        for (Map<String, Double> eachUnit : listOfUnit) {
            for (Map.Entry<String, Double> entry : eachUnit.entrySet()) {
                if (entry.getValue() > maxHours) {
                    maxHours = entry.getValue();
                    maxHoursTopic = entry.getKey();

                }
                if (entry.getValue() < minHours){
                    minHours = entry.getValue();
                    minHoursTopic = entry.getKey();

                }
            }
        }

        System.out.println("maxHoursTopic = " + maxHoursTopic);
        System.out.println("minHoursTopic = " + minHoursTopic);
    }
